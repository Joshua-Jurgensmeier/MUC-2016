...
void addPath(ubyte source, ubyte destination, char direction, ubyte distance, ubyte parkingLot, ubyte startLot)
{
	...
	{
		vertices[source].rightPath = pathCounter;
	}
...
void removePath(ubyte source, ubyte destination)
{
	...
	else if (paths[vertices[source].rightPath].destination == destination)
	{
		vertices[source].leftPath = 0;
	}
}
...
void shortestPath()
{
	...
	permutationPaths[2][0] = locationPaths[2];
	permutationLocations[2][0] = locations[2];
	permutationPaths[2][1] = locationPaths[1];
	permutationLocations[2][1] = locations[1];
	permutationPaths[2][2] = locationPaths[3];
	permutationLocations[2][2] = locations[3];

	permutationPaths[3][0] = locationPaths[2];
	permutationLocations[3][0] = locations[2];
	permutationPaths[3][1] = locationPaths[3];
	permutationLocations[3][1] = locations[3];
	permutationPaths[3][2] = locationPaths[1];
	permutationLocations[3][2] = locations[1];

	permutationPaths[4][0] = locationPaths[3];
	permutationLocations[4][0] = locations[3];
	permutationPaths[4][1] = locationPaths[2];
	permutationLocations[4][1] = locations[2];
	permutationPaths[4][2] = locationPaths[1];
	permutationLocations[4][2] = locations[1];

	permutationPaths[5][0] = locationPaths[3];
	permutationLocations[5][0] = locations[3];
	permutationPaths[5][1] = locationPaths[1];
	permutationLocations[5][1] = locations[1];
	permutationPaths[5][2] = locationPaths[2];
	...
	ubyte totalDists[6];	// Stores the total distances of the route permutations
	...
	for (ubyte i = 0; i < 6 && permutationPaths[i][0] != 0; i++)
	{
		...
		totalDists[i] += dijkstra(paths[permutationPaths[i][x]].destination, paths[locationPaths[4]].destination);

		writeDebugStreamLine("%d %d",paths[permutationPaths[i][x]].destination, paths[locationPaths[4]].destination);
	}
	...
	writeDebugStreamLine("Shortest route number: %d",minIndex);
	writeDebugStreamLine("");
	...
	dijkstra(paths[permutationPaths[minIndex][x]].destination, paths[locationPaths[4]].destination);
